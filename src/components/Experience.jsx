      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center mb-8">
            <div className="font-mono text-white/30 text-lg">{'{'} work_history: [ ]</div>
          </div>
        </div>
        
        <div className="relative">
          <div className="absolute left-1/2 transform -translate-x-1/2 w-0.5 h-full bg-gradient-to-b from-transparent via-white/40 to-transparent rounded-full opacity-60"></div>
          ...
          {experiences.map((exp, index) => (
            <div key={index} className={`relative flex items-center mb-16 ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
              ...
              {/* Year connector */}
              <div className="absolute left-1/2 transform -translate-x-1/2 bg-white/20 backdrop-blur-xl px-4 py-2 rounded-full border border-white/30 text-white font-bold text-sm shadow-lg">
                {exp.year}
              </div>
            </div>
          ))}
